...
inline int build_bins(vector<bin_t1>& bins, particle_t* particles, int n)
{
...
inline int compute_forces_for_bin(vector<bin_t1>& bins, int i, int j, double& dmin, double& davg, int& navg)
{
    bin_t1& vec = bins[i * bin_count + j];
    ...
    for (int dx = -1; dx <= 1; dx++)   //Search over nearby 8 bins and itself
    {
        for (int dy = -1; dy <= 1; dy++)
        {
            ...
            {
                bin_t1& vec2 = bins[(i+dx) * bin_count + j + dy];
                for (int k = 0; k < vec.size(); k++)
...
int bin_particle(particle_t& particle, vector<bin_t1>& bins)
{
...
inline int get_neighbors(int i, int j, vector<int>& neighbors)
{
...
{    
    ...
    */
    ...
    for( int step = 0; step < NSTEPS; step++ )
    {
        ...
        vector<bin_t1> scatter_particles;
        ...
        bin_t1 outgoing_move;
        outgoing_move.resize(send_count);

        bin_t1 scatter_particles_flatten;
